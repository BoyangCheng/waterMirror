...
function Call({ interview }: InterviewProps) {
  ...
  useEffect(() => {
    const fetchInterviewer = async () => {
      ...
      if (interviewer) setInterviewerImg(interviewer.image);
    };
    fetchInterviewer();
  ...
  const startConversation = async () => {
    setLoading(true);
    try {
      // biome-ignore lint/suspicious/noExplicitAny: postgres Row type
      const oldUserEmails: string[] = (await getAllEmails(interview.id) as any[]).map((item: { email: string }) => item.email);
      const isOld =
